...
def BIC(X, kmin=1, kmax=None, iterations=1000, tol=1e-5, verbose=False):
    ...
    for idx, k in enumerate(k_range):
        ...
        # For GMM with full covariance:
        # - k-1 for priors (sum to 1 constraint)
        # - k*d for means
        # - k*d*(d+1)/2 for covariance matrices
        p = (k - 1) + (k * d) + (k * d * (d + 1) // 2)
        
        # Calculate BIC: BIC = p * ln(n) - 2 * log_likelihood
        bic = p * np.log(n) - 2 * lik
        bic_vals[idx] = bic
        
        # Lower BIC is better (balances fit and complexity)
        if bic < best_bic:
            best_bic = bic
            best_k_val = k
