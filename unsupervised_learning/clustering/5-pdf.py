...
def pdf(X, m, S):
    ...
    if (not isinstance(X, np.ndarray) or len(X.shape) != 2 or
        ...
        X.shape[1] != m.shape[0] or X.shape[1] != S.shape[0] or
        S.shape[0] != S.shape[1]):
        return None
    ...
    diff = X - m  # Broadcasting works: (n,d) - (d,) = (n,d)
    
    # Calculate quadratic form: (x-μ)^T Σ^{-1} (x-μ) for each x
    # Using einsum for efficient computation without diagonal
    # diff[i] @ S_inv @ diff[i]^T for each i
